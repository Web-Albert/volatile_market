...
ADX_HIGH_THRESHOLD = 40     # ADX高阈值（趋势较强）

# 合成信号阈值参数（20 <= ADX <= 40时 - 中等趋势）
MEDIUM_TREND_BUY_THRESHOLD = 1.3    # 中等趋势买入阈值
...
def get_volatile_strategy_params():
    ...
    return {
        ...
        'medium_trend_support_buy_ratio': MEDIUM_TREND_SUPPORT_BUY_RATIO,
        'medium_trend_resistance_sell_ratio': MEDIUM_TREND_RESISTANCE_SELL_RATIO,
        'strong_trend_support_buy_ratio': STRONG_TREND_SUPPORT_BUY_RATIO,
        'strong_trend_resistance_sell_ratio': STRONG_TREND_RESISTANCE_SELL_RATIO,
...
def print_volatile_config():
    ...
    print("\n📈 MA支撑压力位参数:")
    print(f"  MA周期: {MA_PERIODS}")
    print(f"  中等趋势支撑位加仓比例: {MEDIUM_TREND_SUPPORT_BUY_RATIO:.2%}")
    print(f"  中等趋势压力位减仓比例: {MEDIUM_TREND_RESISTANCE_SELL_RATIO:.2%}")
    print(f"  强趋势支撑位加仓比例: {STRONG_TREND_SUPPORT_BUY_RATIO:.2%}")
    print(f"  强趋势压力位减仓比例: {STRONG_TREND_RESISTANCE_SELL_RATIO:.2%}")
